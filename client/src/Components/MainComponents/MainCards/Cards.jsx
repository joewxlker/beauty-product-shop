...
const Card = ({info}) => {
    ...
    return (
        <>
            ...
            {!hover[info.title] ? (
                <>
                <div id='main-card' style={{ background: `url(` + info.image + `)` }} className='background-one' onMouseEnter={e => { e.preventDefault(); setHover({[info.title]: true}) }}>
            ...
            ) : (<div id='main-card-hover' style={{ background: `url(` + info.image + `)` }} className='background-hover-one' onMouseLeave={e => { e.preventDefault(); setHover({[info.title]: false}) }}>
